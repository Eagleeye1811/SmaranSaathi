# Reads frontend/.env (gitignored, real secrets) and launches `flutter run`
# with those values passed through as --dart-define flags — Flutter has no
# built-in .env reader, so this script is the bridge. Safe to commit: it
# holds no secrets itself, only the logic to read them from .env.
#
