...
class _PersonStepState extends State<PersonStep> with OnboardingStep<PersonStep> {
  ...
  /// Every helper this screen offers, "myself" excluded — this onboarding is
  /// always reached by choosing "Caregiver" on the authentication screen, so
  /// the person filling it in is never the patient answering for themself.
  static const List<HelperRole> _helperRoles = <HelperRole>[
    HelperRole.spouse,
    HelperRole.child,
    HelperRole.otherFamily,
    HelperRole.professionalCaregiver,
    HelperRole.other,
  ];
  ...
  void _save() {
    // The caregiver's own name was already given when the account was
    // created — asking again here was a second question for the same
    // answer, so it is taken from the account instead.
    edit((OnboardingRecord d) =>
        d.copyWith(caregiverName: state.accountDisplayName?.trim() ?? ''));
    ...
    return IntakeScaffold(
      ...
      voiceQuestions: <VoiceIntakeQuestion>[
        ...
        VoiceIntakeQuestion(
          ...
          options: _helperRoles.map((HelperRole r) => helperRoleLabel(l, r)).toList(growable: false),
          answeredIndex: draft.helper == null ? null : _helperRoles.indexOf(draft.helper!),
          onSelect: (int i) =>
              edit((OnboardingRecord d) => d.copyWith(helper: _helperRoles[i])),
        ),
      ],
      ...
      children: <Widget>[
        ...
        for (final HelperRole r in _helperRoles)
          ChoiceTile(
